...
#define SIZE_OF_GRID 1200
#define SIZe_OF_SPACECRAFT 100
...
int main()
{
	...
	int opcode = 0, counter = 0, result = 0;
	...
	char *grid[SIZE_OF_GRID];
	bool spacecraftSizeFound = false , done = false;
	...
	for (int yy = 0; yy < SIZE_OF_GRID; yy++)
	{
		...
		if (yy > SIZe_OF_SPACECRAFT && done==false)
		{
			while (grid[yy][counter] != '#')
				counter++;

			spacecraftSizeFound = false;
			if (grid[yy][counter] == '#' && grid[yy][counter + SIZe_OF_SPACECRAFT-1] == '#' && grid[yy - SIZe_OF_SPACECRAFT+1][counter + SIZe_OF_SPACECRAFT-1] == '#')
			{
				spacecraftSizeFound = true;
				for (i = 0; i < SIZe_OF_SPACECRAFT; i++)
				{
					if (grid[yy][counter + i] != '#')
						spacecraftSizeFound = false;
					if (grid[yy-i][counter+ SIZe_OF_SPACECRAFT-1] != '#')
						spacecraftSizeFound = false;
					if (grid[yy- SIZe_OF_SPACECRAFT+1][counter + i] != '#')
						spacecraftSizeFound = false;
					if (grid[yy-i][counter] != '#')
						spacecraftSizeFound = false;

					if (spacecraftSizeFound == false)
						i = SIZe_OF_SPACECRAFT;
				}
			}

			if (spacecraftSizeFound == true)
			{
				for (int a = yy; a > (yy - SIZe_OF_SPACECRAFT); a--)
					for (int b = counter; b < (counter + SIZe_OF_SPACECRAFT); b++)
						grid[a][b] = '0';
				
				done = true;
				result = counter * 10000 + (yy - SIZe_OF_SPACECRAFT + 1);
			}
		}
	}
